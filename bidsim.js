  ...
  Network.requestIntercepted(
    async ({interceptionId, request}) => {
      ...
      if ('imp' in ixRequest && ixRequest !== 'undefined') {
        ...
        ixRequest.imp.forEach((element) =>
          'banner' in element ? allOpportunities.push({type: 'banner', impid: element.id, width: element.banner.w, height: element.banner.h, sizesig: element.banner.w +"x"+element.banner.h, hasbid: 0, fake: false}) :
      ...
      execStage[1] = function () {}

      execStage[2] = function () {
        // Let's check how many opportunities did not receive bids and then create fake bids if --everything is enable
      ...
      function finalStage() {
        ...
        allOpportunities.forEach((item) => item.hasbid > 0 ? console.log(colors.blue('*' + item.type, ' ', item.impid, '\t', item.sizesig, '\tbids:' + item.hasbid + '\tFake:' + item.fake)) :
...
function defaultSeatBidFragment() {
  let obj = {
    seat: "6272",
    bid: []
  }
